...
export async function submitLevel3(playerId, selectedOption, questionIndex) {
  ...
  let awardedPoints = 0;
  if (isCorrect) {
    // Avoid composite-index dependency for mobile clients by ranking in-memory.
    const rankSnap = await getDocs(query(collection(db, 'answers'), where('level', '==', 3)));
    const earlierCorrect = rankSnap.docs
      .map(d => d.data())
      .filter(a => a.questionIndex === questionIndex && a.isCorrect === true && a.submittedAt)
      .sort((a, b) => a.submittedAt.toMillis() - b.submittedAt.toMillis());
    const rank = earlierCorrect.length + 1;
